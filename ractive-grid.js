  ...
  index__Grid = Ractive.extend({
    ...
    onrender: function () {
      ...
      grid.on("selection", function (e) {
        ...
        if (e.original.keyCode === 38 && current > 0) current--;else if (e.original.keyCode === 40) current++;else if (e.original.keyCode === 13) {
      ...
      grid.on("headerClicked", function (e, i) {
        ...
        if (e.original.ctrlKey && grid.get("multisortable")) {
          nstate = 3;
          if (!!!state || state > 2) nstate = 1;else if (state === 1) nstate = 2;
          ...
          nstate = 3;
          if (!!!state || state > 2) nstate = 1;else if (state === 1) nstate = 2;else {
    ...
    computed: {
      itemsView: function () {
        ...
        items.sort(function (a, b) {
          for (var i in sorts) {
            var s = sorts[i];
            var dir = 1;
            var ai = index__getProp(a, columns[s.field].path);
            var bi = index__getProp(b, columns[s.field].path);

            if (s.dir === 2) dir = -1;else if (s.dir > 2) continue;

            if (ai < bi) return -1 * dir;else if (ai > bi) return 1 * dir;else continue;
          }
          return 0;
        });
        return items;
      ...
      headers: function () {
        ...
        for (var i in cols) {
          var c = cols[i];
          if (!!c.label) res.push(c.label);else if (!!!c.compute) {
    ...
    filterInputControl: function () {
      ...
      return function (e) {
        ...
        if (k === 38 || k === 40) {
          e.original.preventDefault();
          var current = grid.get("currentIndex");
          if (typeof current !== "number") current = k === 38 ? grid.get("itemsView").length - 1 : 0;else current = k == 38 ? current - 1 : current + 1;
          grid.current(current);
          return;
        }
      };
